...
| fetched_at | TIMESTAMP | この場所情報を取得した日時 |
...
営業時間や所在地は現実側が変わるため、placesは更新されうる。ただし過去の旅程が組まれた時点の営業時間までは保存しない(Phase 1では過剰)。代わりにfetched_atを持ち、「営業時間(2026-09-17時点)」のように鮮度を表示してユーザーが判断できるようにする。
